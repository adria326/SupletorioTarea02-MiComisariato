# Variables globales
productos = []  # Lista para almacenar productos de despensa
...
def gestionar_productos():
    while True:
        ...
        if opcion == '1':
            ...
            eliminar_producto()  # Llamada a la función para eliminar un producto
        elif opcion == '4':
            break
        else:
...
def gestionar_usuarios():
    while True:
        ...
        if opcion == '1':
            ...
            editar_usuario()  # Llamada a la función para editar un usuario
        elif opcion == '4':
            break
        else:
            print("Opción no válida, por favor intente de nuevo.")
...
def editar_usuario():
    ...
    try:
        ...
        if 0 <= usuario_index < len(usuarios):
            ...
            print(f"Editando usuario: {usuario['nombre']} - Cédula: {usuario['cedula']}")
            nuevo_nombre = input("Ingrese el nuevo nombre (deje en blanco para mantener el actual): ")
            nueva_cedula = input("Ingrese la nueva cédula (deje en blanco para mantener la actual): ")
            ...
            if nueva_cedula:
                if len(nueva_cedula) == 10 and nueva_cedula.isdigit():
                    ...
                    print("Cédula no válida. Manteniendo la cédula actual.")
...
# Función para generar una factura de venta
def generar_factura():
    if not productos:
...
# Bucle principal para ejecutar el sistema
while True:
    opcion = menu_principal()  # Llamada a la función y uso del valor retornado
    if opcion == '1':
        gestionar_productos()
    elif opcion == '2':
        gestionar_usuarios()
